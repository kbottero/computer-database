package com.excilys.cdb.page.mapper.impl;
...
public class ComputerPageMapper extends IPageMapper<Computer,Long> {
	...
	public static Page<Computer,Long> fromDTO (IService<Computer,Long> service, ComputerPageDTO pageDTO) {
		...
		if (sort != null) {
			for (Iterator<Order> it = sort.iterator(); it.hasNext();) {
				...
				if (order.getDirection().equals(Direction.ASC)) {
					orderMap.put(ComputerDao.mapBDModel.get(order.getProperty()), DaoRequestParameter.Order.ASC);
				} else {
					orderMap.put(ComputerDao.mapBDModel.get(order.getProperty()), DaoRequestParameter.Order.DESC);
				}
			}
		}
